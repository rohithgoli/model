...
def display_tasks_by_mentor():
    if current_user.get_user_type() == 'Mentor':
        ...
        flash(f'Access denied !! You are logged out, Please access using authorised credentials', 'danger')
        return render_template('home.html')
# F:JKF:SAJKF:LKF;SAKF:AK:Lskf;lka;f

    if request.method == 'POST':
        mentor = request.form.get('mentor')
        if mentor == "Select Mentor":
            flash('Please Choose Mentor', 'danger')
            return render_template('tasks-by-mentor.html', mentors=current_mentor_list)
        raw_list = mentor.split("'")
        requested_mentor_email = [item for item in raw_list if "@gmail.com" in item]
        requested_mentor_email = requested_mentor_email[0]
        with client.context():
            requested_mentor = Mentor.query().filter(Mentor.email == requested_mentor_email).get()
            requested_mentor_key = requested_mentor.key
            requested_tasks = Task.query().filter(Task.assigned_by == requested_mentor_key).fetch()
            # print(requested_tasks)
            if len(requested_tasks) == 0:
                ...
                for each_task in requested_tasks:
                    ...
                    task_dict['Content'] = each_task_details.content
                    required_tasks_data.append(task_dict)
                flash(f'Total {required_tasks_count} tasks were assigned by {requested_mentor}', 'info')
                return render_template('tasks-by-mentor.html', mentors=current_mentor_list, tasks=required_tasks)
    else:
        return render_template('tasks-by-mentor.html', mentors=current_mentor_list)
...
def display_tasks():
    if current_user.get_user_type() == "Admin":
        ...
        """
        with client.context():
            intern = Intern.query().filter(Intern.email == current_user.email).get()
            intern_tasks = Task.query().filter(Task.assigned_to == intern.key).fetch()
            tasks = intern_tasks
            task_list = []
            for each_task in tasks:
                ...
                creator = ndb.Key(Mentor, each_task.assigned_by.id()).get().username
                task_dict['assigned_by'] = creator
                task_list.append(task_dict)
        return render_template('tasks.html', title='Your Tasks', tasks=task_list)
        """
...
# FIX IT     FIX IT      IN PROCESS -----------------------------      FIX IT      FIX IT      FIX IT
@app.route("/post/<int:post_id>")
def post(post_id):
    with client.context():
        post = Post.query().get_by_id(post_id)
        print(post)
